...
test('migration preserves legacy dispensing facts and makes unclassified products explicit', async () => {
  ...
  try {
    ...
    for (const name of migrations.filter(n => !n.startsWith('20260909120000'))) await pg.exec(fs.readFileSync(path.join(root, name, 'migration.sql'), 'utf8'));
    await pg.exec(`
      INSERT INTO "User" ("id","email","password","updatedAt") VALUES ('p','p@example.test','test',NOW()),('d','d@example.test','test',NOW());
      INSERT INTO "Medicament" ("id","cis","nom","forme") VALUES ('m','1','Produit test','Test');
    ...
    `);
    await pg.exec(fs.readFileSync(path.join(root, '20260909120000_role_prescription_workflows/migration.sql'), 'utf8'));
